public class NeptuneData
{
    static public string[] Stars = {
        "BOB 1    ", "BOB 2    ", "BOB 3    ", "BOB 4    ", "BOB 5    ", "BOB 6    ", "BOB  100C", 
        "WF  1    ", "WF  2    ", "WF  3    ", "WF  4    ", "WF  5    ", "WF  6    ", "WF   100C", 
        "JRB 1    ", "JRB 2    ", "JRB 3    ", "JRB 4    ", "JRB 5    ", "JRB 6    ", "JRB  100C", 
        "CCM 1    ", "CCM 2    ", "CCM 3    ", "CCM 4    ", "CCM 5    ", "CCM 6    ", "CCM  100C", 
        "BBH 1    ", "BBH 2    ", "BBH 3    ", "BBH 4    ", "BBH 5    ", "BBH 6    ", "BBH  100C", 
        "HMC 1    ", "HMC 2    ", "HMC 3    ", "HMC 4    ", "HMC 5    ", "HMC 6    ", "HMC  100C", 
        "LLL 1    ", "LLL 2    ", "LLL 3    ", "LLL 4    ", "LLL 5    ", "LLL 6    ", "LLL  100C", 
        "SSL 1    ", "SSL 2    ", "SSL 3    ", "SSL 4    ", "SSL 5    ", "SSL 6    ", "SSL  100C", 
        "DDD 1    ", "DDD 2    ", "DDD 3    ", "DDD 4    ", "DDD 5    ", "DDD 6    ", "DDD  100C", 
        "SL  1    ", "SL  2    ", "SL  3    ", "SL  4    ", "SL  5    ", "SL  6    ", "SL   100C", 
        "WDW 1    ", "WDW 2    ", "WDW 3    ", "WDW 4    ", "WDW 5    ", "WDW 6    ", "WDW  100C", 
        "TTM 1    ", "TTM 2    ", "TTM 3    ", "TTM 4    ", "TTM 5    ", "TTM 6    ", "TTM  100C", 
        "THI 1    ", "THI 2    ", "THI 3    ", "THI 4    ", "THI 5    ", "THI 6    ", "THI  100C", 
        "TTC 1    ", "TTC 2    ", "TTC 3    ", "TTC 4    ", "TTC 5    ", "TTC 6    ", "TTC  100C", 
        "RR  1    ", "RR  2    ", "RR  3    ", "RR  4    ", "RR  5    ", "RR  6    ", "RR   100C", 
        "PSS     N", "PSS     F", "SA       ", "TOTWC    ", "COTMC    ", "VCUTM    ", "WMOTR    ",
        "MIPS    1", "MIPS    2", "TOAD    1", "TOAD    2", "TOAD    3", "BITDW    ", "BITFS    ", "BITS     "
    };

    static public string[] LoggedAbbrs = {
        "BoB 1", "BoB 2", "BoB 3", "BoB 4", "BoB 5", "BoB 6", "BoB 100C", 
        "WF 1", "WF 2", "WF 3", "WF 4", "WF 5", "WF 6", "WF 100C", 
        "JRB 1", "JRB 2", "JRB 3", "JRB 4", "JRB 5", "JRB 6", "JRB 100C", 
        "CCM 1", "CCM 2", "CCM 3", "CCM 4", "CCM 5", "CCM 6", "CCM 100C", 
        "BBH 1", "BBH 2", "BBH 3", "BBH 4", "BBH 5", "BBH 6", "BBH 100C", 
        "HMC 1", "HMC 2", "HMC 3", "HMC 4", "HMC 5", "HMC 6", "HMC 100C", 
        "LLL 1", "LLL 2", "LLL 3", "LLL 4", "LLL 5", "LLL 6", "LLL 100C", 
        "SSL 1", "SSL 2", "SSL 3", "SSL 4", "SSL 5", "SSL 6", "SSL 100C", 
        "DDD 1", "DDD 2", "DDD 3", "DDD 4", "DDD 5", "DDD 6", "DDD 100C", 
        "SL 1", "SL 2", "SL 3", "SL 4", "SL 5", "SL 6", "SL 100C", 
        "WDW 1", "WDW 2", "WDW 3", "WDW 4", "WDW 5", "WDW 6", "WDW 100C", 
        "TTM 1", "TTM 2", "TTM 3", "TTM 4", "TTM 5", "TTM 6", "TTM 100C", 
        "THI 1", "THI 2", "THI 3", "THI 4", "THI 5", "THI 6", "THI 100C", 
        "TTC 1", "TTC 2", "TTC 3", "TTC 4", "TTC 5", "TTC 6", "TTC 100C", 
        "RR 1", "RR 2", "RR 3", "RR 4", "RR 5", "RR 6", "RR 100C", 
        "PSS N", "PSS F", "SA", "TotWC", "CotMC", "VCutM", "WMotR",
        "MIPS 1", "MIPS 2", "Toad 1", "Toad 2", "Toad 3", "BitDW", "BitFS", "BitS"
    };

    static public string[] LoggedStars = {
        "Bob-omb Battlefield #1", "Bob-omb Battlefield #2", "Bob-omb Battlefield #3", "Bob-omb Battlefield #4", "Bob-omb Battlefield #5", "Bob-omb Battlefield #6", "Bob-omb Battlefield 100 Coins", 
        "Whomp's Fortress #1", "Whomp's Fortress #2", "Whomp's Fortress #3", "Whomp's Fortress #4", "Whomp's Fortress #5", "Whomp's Fortress #6", "Whomp's Fortress 100 Coins", 
        "Jolly Roger Bay #1", "Jolly Roger Bay #2", "Jolly Roger Bay #3", "Jolly Roger Bay #4", "Jolly Roger Bay #5", "Jolly Roger Bay #6", "Jolly Roger Bay 100 Coins", 
        "Cool Cool Mountain #1", "Cool Cool Mountain #2", "Cool Cool Mountain #3", "Cool Cool Mountain #4", "Cool Cool Mountain #5", "Cool Cool Mountain #6", "Cool Cool Mountain 100 Coins", 
        "Big Boo's Haunt #1", "Big Boo's Haunt #2", "Big Boo's Haunt #3", "Big Boo's Haunt #4", "Big Boo's Haunt #5", "Big Boo's Haunt #6", "Big Boo's Haunt 100 Coins", 
        "Hazy Maze Cave #1", "Hazy Maze Cave #2", "Hazy Maze Cave #3", "Hazy Maze Cave #4", "Hazy Maze Cave #5", "Hazy Maze Cave #6", "Hazy Maze Cave 100 Coins", 
        "Lethal Lava Land #1", "Lethal Lava Land #2", "Lethal Lava Land #3", "Lethal Lava Land #4", "Lethal Lava Land #5", "Lethal Lava Land #6", "Lethal Lava Land 100 Coins", 
        "Shifting Sand Land #1", "Shifting Sand Land #2", "Shifting Sand Land #3", "Shifting Sand Land #4", "Shifting Sand Land #5", "Shifting Sand Land #6", "Shifting Sand Land 100 Coins", 
        "Dire Dire Docks #1", "Dire Dire Docks #2", "Dire Dire Docks #3", "Dire Dire Docks #4", "Dire Dire Docks #5", "Dire Dire Docks #6", "Dire Dire Docks 100 Coins", 
        "Snowman's Land #1", "Snowman's Land #2", "Snowman's Land #3", "Snowman's Land #4", "Snowman's Land #5", "Snowman's Land #6", "Snowman's Land 100 Coins", 
        "Wet Dry World #1", "Wet Dry World #2", "Wet Dry World #3", "Wet Dry World #4", "Wet Dry World #5", "Wet Dry World #6", "Wet Dry World 100 Coins", 
        "Tall Tall Mountain #1", "Tall Tall Mountain #2", "Tall Tall Mountain #3", "Tall Tall Mountain #4", "Tall Tall Mountain #5", "Tall Tall Mountain #6", "Tall Tall Mountain 100 Coins", 
        "Tiny Huge Island #1", "Tiny Huge Island #2", "Tiny Huge Island #3", "Tiny Huge Island #4", "Tiny Huge Island #5", "Tiny Huge Island #6", "Tiny Huge Island 100 Coins", 
        "Tick Tock Clock #1", "Tick Tock Clock #2", "Tick Tock Clock #3", "Tick Tock Clock #4", "Tick Tock Clock #5", "Tick Tock Clock #6", "Tick Tock Clock 100 Coins", 
        "Rainbow Ride #1", "Rainbow Ride #2", "Rainbow Ride #3", "Rainbow Ride #4", "Rainbow Ride #5", "Rainbow Ride #6", "Rainbow Ride 100 Coins", 
        "Princess' Secret Slide (Normal)", "Princess' Secret Slide (Fast)", "Secret Aquarium", "Tower of the Wing Cap", "Cavern of the Metal Cap", "Vanish Cap under the Moat", "Wing Mario over the Rainbow",
        "MIPS #1", "MIPS #2", "Toad #1", "Toad #2", "Toad #3", "Bowser in the Dark World", "Bowser in the Fire Sea", "Bowser in the Sky"
    };

    static public string[][] Enemies = new string[120][] {
        new string[1]{"B+"},
        new string[1]{"Kp+"},
        new string[2]{"G", "B"},
        new string[3]{"G", "B", "CC"},
        new string[2]{"G", "B"},
        new string[2]{"G", "B"},
        new string[3]{"G", "Kp", "B"},
        
        new string[2]{"Wh", "Wh+"},
        new string[4]{"G", "B", "PP", "Wh"},
        new string[0]{}, //↖ Ø ←
        new string[2]{"Wh", "Th"},
        new string[2]{"Wh", "Wh+"},
        new string[0]{}, //← Ø ↗
        new string[2]{"PP", "Wh"},
        
        new string[0]{}, //↘ Ø ↗
        new string[2]{"Cl", "Ee"},
        new string[0]{}, //↙ N/A ↖
        new string[1]{"Cl"},
        new string[0]{}, //↘ Ø ↗
        new string[2]{"Cl", "G"},
        new string[2]{"Cl", "G"},
        
        new string[0]{}, //→ Ø ↘
        new string[1]{"Pg-"},
        new string[1]{"Pg+"},
        new string[2]{"Sd", "MB"},
        new string[2]{"Sn", "Sd"},
        new string[1]{"Sd"},
        new string[0]{}, //← Ø ↓
        
        new string[2]{"Bo", "Bo+"},
        new string[4]{"S", "Bk", "Bo", "Bo+"},
        new string[3]{"Bo", "Bo+", "Bk"},
        new string[4]{"Pn", "Bk", "Bo", "Bo+"},
        new string[4]{"S", "Bo", "Bo+", "Bk"},
        new string[5]{"S", "Bo", "Bo+", "Bk", "MI"},
        new string[5]{"MI", "Bo", "Bo+", "Pn", "Bk"},
        
        new string[2]{"MP", "S"},
        new string[2]{"S", "Sw"},
        new string[2]{"S", "Dr"},
        new string[3]{"S", "Sf", "Sw"},
        new string[3]{"S", "Sf", "Sw"},
        new string[0]{}, //↘ N/A ↑
        new string[4]{"S", "MI", "Sw", "Sf"},
        
        new string[1]{"Bl+"},
        new string[2]{"Bl", "Bl+"},
        new string[0]{}, //↓ Ø ↘
        new string[0]{}, //↘ Ø ↓
        new string[0]{}, //↓ Ø ↙
        new string[0]{}, //↙ N/A ↘
        new string[2]{"MI", "Bl"},
        
        new string[1]{"Kl"},
        new string[0]{}, //← Ø →
        new string[0]{}, //↙ N/A →
        new string[5]{"F", "B", "Pk", "G", "Ey"},
        new string[1]{"F"},
        new string[0]{}, //↙ N/A →
        new string[6]{"B", "F", "Pk", "G", "TB", "Gr"},

        new string[0]{}, //↘ Ø ↓
        new string[1]{"Cl"},
        new string[0]{}, //↘ N/A ↗
        new string[0]{}, //↙ Ø ↑
        new string[1]{"MR"},
        new string[0]{}, //↙ Ø ↗
        new string[0]{}, //← N/A ↑

        new string[2]{"Sd", "F"},
        new string[1]{"Bl*"},
        new string[1]{"Sd"},
        new string[1]{"Sd"},
        new string[1]{"Sd"},
        new string[2]{"Sd", "G"},
        new string[4]{"Sd", "MB", "Mn", "F"},

        new string[0]{}, //↗ Ø ↓
        new string[2]{"HH", "Cy"},
        new string[2]{"HH", "Cy"},
        new string[2]{"HH", "Cy"},
        new string[1]{"HH"},
        new string[1]{"HH"},
        new string[3]{"Sk", "HH", "Cy"},

        new string[5]{"Fw", "F", "MM", "B", "G"},
        new string[1]{"Uk"},
        new string[0]{}, //← Ø ↓
        new string[0]{}, //↗ Ø ↙
        new string[0]{}, //↑ Ø ↘
        new string[0]{}, //↑ Ø ↓
        new string[2]{"G", "Cy"},

        new string[2]{"G-", "PP"},
        new string[2]{"G-", "PP"},
        new string[4]{"G-", "PP", "Kp+", "G+"},
        new string[2]{"G-", "PP"},
        new string[0]{}, //↑ N/A →
        new string[4]{"G-", "PP", "Cy", "Wg"},
        new string[2]{"G-", "PP"},

        new string[4]{"HH", "Cy", "B", "FS"},
        new string[3]{"F", "B", "FS"},
        new string[0]{}, //↓ Ø ↘
        new string[0]{}, //↙ N/A ↖
        new string[3]{"F", "B", "FS"},
        new string[0]{}, //→ Ø ↑
        new string[1]{"B"},

        new string[2]{"F", "Lk"},
        new string[3]{"F", "Lk", "B"},
        new string[2]{"F", "Lk"},
        new string[4]{"Lk", "B", "F", "G"},
        new string[3]{"Lk", "B", "F"},
        new string[3]{"F", "Lk", "Cy"},
        new string[3]{"F", "Lk", "B"},

        new string[0]{}, //→ N/A →
        new string[0]{}, //→ Ø →
        new string[6]{"B", "G", "Bw", "F", "Pk", "FS"},
        new string[0]{}, //→ Ø ←
        new string[3]{"S", "Dr", "Sf"},
        new string[0]{}, //← Ø ←
        new string[0]{}, //← N/A ←

        new string[1]{"MP"},
        new string[1]{"MP"},
        new string[0]{}, //← Ø ←
        new string[0]{}, //← Ø ←
        new string[0]{}, //← Ø ←

        new string[2]{"G", "Bw"},
        new string[0]{}, //← N/A →
        new string[6]{"F", "Lk", "B", "FS", "G", "Bw"}
    };

    static public bool[] HasEnemies = {
        true,  true,  true,  true,  true,  true,  true,  
        true,  true,  false, true,  true,  false, true,  
        false, true,  false, true,  false, true,  true,  
        false, true,  true,  true,  true,  true,  false, 
        true,  true,  true,  true,  true,  true,  true,  
        true,  true,  true,  true,  true,  false, true,  
        true,  true,  false, false, false, false, true,  
        true,  false, false, true,  true,  false, true,  
        false, true,  false, false, true,  false, false, 
        true,  true,  true,  true,  true,  true,  true,  
        false, true,  true,  true,  true,  true,  true,  
        true,  true,  false, false, false, false, true,  
        true,  true,  true,  true,  false, true,  true,  
        true,  true,  false, false, true,  false, true,  
        true,  true,  true,  true,  true,  true,  true,  
        false, false, true,  false, true,  false, false,
        true,  true,  false, false, false,  
        true,  false, true    
    };

    static public int[] NoEnemies = { 9, 12, 14, 16, 18, 21, 27, 40, 44, 45, 46, 47, 50, 51, 54, 56, 58, 59, 61, 62, 70, 79, 80, 81, 82, 88,  93,  94, 96, 105, 106, 108, 110, 111, 114, 115, 116, 118};
     static public int[] OddArrow = { 1, 11, 22, 22, 26, 22, 26, 48, 65, 53, 53, 53, 49, 57, 60, 64, 66, 65, 67, 60, 64, 78, 74, 74, 75, 74, 100, 100, 97, 107, 107, 109, 109, 109, 113, 113, 113, 117};
    static public int[] EvenArrow = { 8,  6,  8,  8,  6, 29, 34, 33, 52, 52, 52, 55, 52, 52, 55, 63, 52, 62, 55, 55, 77, 86, 86, 89, 89, 89, 101,  86, 89, 107, 107, 107, 109, 109, 113, 113, 113, 119};
}