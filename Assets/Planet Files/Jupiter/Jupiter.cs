...
/*************************************************************************************
This version of the code was created by Deaf and released on GitHub on April 4th 2021.
 Due to it being buggy when testing it this is NOT the code used in the final module.
  See jupiterScript.cs for the actual source code used in the module.
*************************************************************************************/
...
public class Jupiter : MonoBehaviour {
   ...
   void Start () {
      ...
      //Coroutine PlanetRotating = StartCoroutine(PlanetRotation()); //According to Unity This is never used but this is false, just commenting out so I don't get the message.
   ...
   bool Calculate (string Parameter) { //Calculates each condition as needed
      ...
      switch (Parameter) {
         ...
         case "J":
            ...
            break;
         case "K":
            if (Visits == 9 || "A2B1C2C3D1E1E2F1G2".Contains(PreviousNode)) {
               Temp = true;
            }
            break;
      }
