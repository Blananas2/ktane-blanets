...
public class MarsScript : MonoBehaviour { //depends on name
    ...
    private IEnumerator PlanetRotation() {
        var elapsed = 0f;
        while (true) {
            Planet.transform.localEulerAngles = new Vector3(elapsed / 45 * 360, 90f, 90f); //depends on time it takes to go around in 1 day
            yield return null;
    ...
    void PlanetButtonPress(KMSelectable PlanetButton) {
        for (int i = 0; i < 6; i++) {
            if (PlanetButtons[i] == PlanetButton) {
                ...
                moduleSolved = true;
                Debug.LogFormat("[Mars #{0}] Pressed {1}, module solved.", moduleId, i);
            }
        }
    }
