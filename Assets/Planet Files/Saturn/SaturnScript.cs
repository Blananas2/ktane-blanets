...
public class SaturnScript : MonoBehaviour { //depends on name
    ...
    private IEnumerator PlanetRotation() {
        var elapsed = 0f;
        while (true) {
            Planet.transform.localEulerAngles = new Vector3(elapsed / 706 * 360, 90f, 90f); //depends on time it takes to go around in 1 day
            yield return null;
    ...
    private IEnumerator HidePlanet() {
        for (int i = 0; i < 25; i++) {
            ...
            Background.transform.localScale += new Vector3(0.02f, 0.02f, 0.02f); //depends on size of the planet
        }
        Visible = !Visible;
        Planet.SetActive(Visible);
        for (int i = 0; i < 25; i++) {
            yield return new WaitForSeconds(0.05f);
            Background.transform.localScale -= new Vector3(0.02f, 0.02f, 0.02f); //see above
        }
        Debug.LogFormat("<Saturn #{0}> Visible toggled to {1}.", moduleId, Visible);
        yield return null;
    }
    ...
    void PlanetButtonPress(KMSelectable PlanetButton) {
        for (int i = 0; i < 6; i++) {
            if (PlanetButtons[i] == PlanetButton) {
                ...
                moduleSolved = true;
                Debug.LogFormat("[Saturn #{0}] Pressed {1}, module solved.", moduleId, i);
            }
        }
    }
